...
@mcp.tool()
def send_my_mail(to: str, subject: str, body: str) -> str:
    """Send an email using the user's API (demo version).
    
    For demo purposes, this tool returns a formatted string with the email details
    instead of actually sending the email, to confirm successful tool invocation.
    ...
    Output:
        confirmation: str -> formatted string containing the email details for demo purposes
    """
    
    print(f"DEBUG: To='{to}', Subject='{subject}', Body length={len(body)}")
    
    # Validate inputs
    if not isinstance(to, str) or not isinstance(subject, str) or not isinstance(body, str):
        raise ValueError("All parameters (to, subject, body) must be strings")
    ...
    # For demo purposes, return formatted email details
    demo_response = f"""Email successfully queued for sending!

To: {to}
Subject: {subject}
Body: {body}

[This is a demo response - no actual email was sent]"""
    
    print(f"DEBUG: Returning demo response")
